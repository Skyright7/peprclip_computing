...
# https://pytorch.org/docs/stable/distributions.html 后续预计是准备在这些不同的分布中选择喜欢的分布方式做定制
...
parser = argparse.ArgumentParser(description='generating model of poisson version.')
...
# lambda_value = 0.3 # 设置一个适当的lambda值
# sample_variances = range(5,22, 4)

def generate_pep_dict_poisson(base_file_path,num_base_peps,num_peps_per_base,min_length,max_length,lambda_value,sample_variances):
    ...
    for pep in tqdm(sampled_peptides):
        ...
        for i in sample_variances:
            for j in range(num_samples_per_base):
                gen_pep = token_representations + torch.poisson(
                    torch.full(token_representations.shape, lambda_value)) * i * token_representations.var()
...
out_list = generate_pep_dict_poisson(base_file_path, num_base_peps, num_peps_per_base, min_length, max_length, lambda_value,sample_variances)
...
#     num_base_peps = 10  # 在候选集中随机选出几个多肽作为基底
#     num_peps_per_base = 100  # 每个sample为底生成几个候选多肽
#     min_length = 15  # 生成的多肽的最短长度
#     max_length = 18  # 生成的多肽的最长长度
#     lambda_value = 0.5 # 设置一个适当的lambda值
#     sample_variances = range(5, 22, 4)
#     output_path = 'data/generate_demo_data/output/generated_peptides_poisson_variances.csv'
#     out_list = generate_pep_dict_poisson(base_file_path, num_base_peps, num_peps_per_base, min_length, max_length, lambda_value,sample_variances)
#     df = pd.DataFrame(out_list,columns=['generated_peptides'])
#     df.to_csv(output_path)
